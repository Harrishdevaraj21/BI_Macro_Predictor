...
MODEL_URLS = {
    "xgb_bi_macro_predictor.pkl": "https://drive.google.com/uc?export=download&id=14TJE_LSIakQHcIa3cIFRRB-Ig0RLl5ym",
    "var_bi_macro_predictor.pkl": "https://drive.google.com/uc?export=download&id=14TJE_LSIakQHcIa3cIFRRB-Ig0RLl5ym",
    "finbert_sentiment_components.pkl": "https://drive.google.com/uc?export=download&id=1mm7SFK_rYk-GfPJGQPb0HPCA5wwyxGxc",
}
...
def load_models():
    ...
    try:
        for filename, url in MODEL_URLS.items():
            if not os.path.exists(filename) or os.path.getsize(filename) < 5_000_000:
        ...
        # Safety check: ensure files are real binaries, not HTML
        for filename in MODEL_URLS.keys():
            with open(filename, "rb") as f:
                if f.read(5).startswith(b"<"):
                    raise ValueError(f"{filename} is HTML, not a pickle file")
